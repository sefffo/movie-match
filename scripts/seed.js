...
const USERS = [
  { id: randomUUID(), name: "Saif",    avatarColor: "#e94560" },
  { id: randomUUID(), name: "Maya",    avatarColor: "#0f3460" },
];
...
// Define which user index likes / wants to watch which movie indices
const LIKES = [
  [0, [0, 1, 2]],   // Saif liked: Inception, Interstellar, Parasite
  [1, [3, 4, 6]],   // Maya liked:  Knives Out, La La Land, Spirited Away
];

const WATCHLIST = [
  [0, [1, 3, 8, 9]],  // Saif wants: Interstellar, Knives Out, EEAAO, Arrival
  [1, [1, 3, 7, 9]],  // Maya wants: Interstellar, Knives Out, Grand Budapest, Arrival
];
...
async function seed() {
  ...
  try {
    ...
    for (const u of USERS) {
      await session.run(
        "MERGE (u:User {id: $id}) SET u.name = $name, u.avatarColor = $avatarColor, u.createdAt = coalesce(u.createdAt, datetime())",
        u
      );
    }
    ...
    for (const m of MOVIES) {
      await session.run(
        "MERGE (m:Movie {id: $id}) SET m.title = $title, m.genre = $genre, m.year = $year, m.createdAt = coalesce(m.createdAt, datetime())",
        m
      );
    }
